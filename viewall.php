...
// v2.0.4 FIX (BUG-AGD-BLOCKBASE): Previous versions loaded block_aigrader_dashboard.php
...
if (empty($courses)) {
    ...
    foreach ($courses as $course) {
        ...
        echo html_writer::tag('span', format_string($course['name']), ['class' => 'agd-viewall-course-name']);
        echo html_writer::tag('span', $course['total_ungraded'] . ' ' . get_string('ungraded', 'block_aigrader_dashboard'),
        ...
        foreach ($course['quizzes'] as $quiz) {
            $row_class = $quiz['is_overdue'] ? 'agd-viewall-row-overdue' : '';
            ...
            echo html_writer::tag('td', $quiz['is_overdue']
                ? html_writer::tag('span', get_string('overdue', 'block_aigrader_dashboard'), ['class' => 'agd-badge agd-badge-danger'])
                : '&mdash;');
            echo html_writer::tag('td',
                html_writer::link($quiz['link'],
        ...
        echo html_writer::end_div(); // agd-viewall-course
    }

    echo html_writer::end_div(); // agd-viewall-courses
}
...
.agd-viewall-course-header { display: flex; align-items: center; gap: .75rem; padding: .75rem 1rem; background: #f8f9fa; border-bottom: 1px solid #dee2e6; flex-wrap: wrap; }
...
.agd-viewall-table th, .agd-viewall-table td { padding: .5rem 1rem !important; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.agd-badge { display: inline-block; padding: .2em .6em; border-radius: 1em; font-size: .8em; font-weight: 600; white-space: nowrap; }
.agd-badge-warning { background: #fff3cd; color: #856404; }
.agd-badge-danger  { background: #f8d7da; color: #721c24; }
.agd-btn { display: inline-flex; align-items: center; gap: .3rem; padding: .35rem .75rem; border-radius: 4px; text-decoration: none; font-size: .85rem; font-weight: 500; cursor: pointer; }
