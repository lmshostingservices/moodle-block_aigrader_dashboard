...
class block_aigrader_dashboard extends block_base {
     ...
     * RC2 fix: capability check is now done before the SQL (in get_gradable_course_ids),
     * so the post-query per-row check is removed entirely.
     * RC3 fix: correlated subquery on question_attempt_steps replaced with LEFT JOIN.
     */
    private function get_ungraded_data(array $gradable_course_ids) {
        ...
        // RC3 fix: LEFT JOIN anti-pattern replaces the correlated subquery that ran
        // SELECT MAX(sequencenumber) once per question_attempt row.
        // IMPORTANT: First column must be unique for get_records_sql() - use CONCAT of course+quiz IDs.
        $sql = "SELECT
